...
impl<BackendData: Backend + 'static> FlutterEngine<BackendData> {
    ...
    pub fn monitor_layout_changed(&mut self, outputs: impl Iterator<Item=Monitor>) {
        self.platform_method_channel.invoke_method("monitor_layout_changed", Some(Box::new(
            EncodableValue::Map(vec![
                (EncodableValue::String("monitors".to_string()), EncodableValue::List(
                    outputs.map(|monitor| EncodableValue::Map(vec![
                        (EncodableValue::String("name".to_string()), EncodableValue::String(monitor.name)),
                        (EncodableValue::String("description".to_string()), EncodableValue::String(monitor.description)),
                        (EncodableValue::String("geometry".to_string()), EncodableValue::Map(vec![
                            (EncodableValue::String("x".to_string()), EncodableValue::Int32(monitor.geometry.loc.x)),
                            (EncodableValue::String("y".to_string()), EncodableValue::Int32(monitor.geometry.loc.y)),
                            (EncodableValue::String("width".to_string()), EncodableValue::Int32(monitor.geometry.size.w)),
                            (EncodableValue::String("height".to_string()), EncodableValue::Int32(monitor.geometry.size.h)),
                        ])),
                    ])).collect::<_>()
                )),
            ]),
