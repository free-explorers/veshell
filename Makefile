...
build:
	VESHELL_LIB_DIR=$(LIBDIR) cargo build --profile=$(PROFILE)
...
package:
	...
	install -Dm644 $(ASSETS_DIR)/veshell.service $(DESTDIR)$(SYSTEMD_USER_DIR)/veshell.service
	...
	# Create data directory and copy contents
	install -d -m 755 $(DESTDIR)$(SHAREDIR)/data
	cp -r $(DATA_DIR)/* $(DESTDIR)$(SHAREDIR)/data/

	# Create settings directory and copy contents
	install -d -m 755 $(DESTDIR)$(SHAREDIR)/settings
...
uninstall:
	...
	# Remove data and settings directories
