use crate::meta_window_state::meta_popup::MetaPopupPatch;
...
pub fn meta_popup_patches<BackendData: Backend + 'static>(
    ...
    let payload: MetaPopupPatch = serde_json::from_value(args).unwrap();

    data.patch_meta_popup(payload, false);
    result.success(None);
}
